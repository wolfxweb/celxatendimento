...
Expected: visible
Error: strict mode violation: locator('aside nav').getByRole('link', { name: /Tickets/i }) resolved to 2 elements:
    1) <a href="/dashboard/cliente/tickets" class="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-300 hover:text-white hover:bg-slate-700/50 transition-all duration-200 group">…</a> aka getByRole('link', { name: '🎫 Meus Tickets', exact: true })
    2) <a href="/dashboard/atendente/tickets" class="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-300 hover:text-white hover:bg-slate-700/50 transition-all duration-200 group">…</a> aka getByRole('link', { name: '📋 Tickets', exact: true })
