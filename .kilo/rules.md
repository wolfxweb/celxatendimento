...
8. Em routers FastAPI, declare rotas estáticas/específicas antes de rotas dinâmicas como `/{id}`. Exemplo: `/tickets/ai/stats` deve vir antes de `/tickets/{ticket_id}`, senão `ai` será tratado como `ticket_id`.
